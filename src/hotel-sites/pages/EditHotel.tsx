...
import formatroom from './formatRoomtype.json'
function EditHotel() {
  const {id} = useParams();
  
  const [dataHotel,setDataHotel] = useState(formatroom); // ใช้รับข้อมูลจาก backend ที่ get มา
 ...
 useEffect(()=>{
  ...
  const sendForm = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const jason = JSON.stringify({
        name: target.name.value == ''? dataHotel.name : target.name.value,
        email: target.email.value == ''? dataHotel.email : target.email.value,
        phone: target.phone.value == ''? dataHotel.phone : target.phone.value,
        address: target.address.value == ''? dataHotel.address : target.address.value,
        about: target.about.value == ''? dataHotel.about : target.about.value,
        mapURL: target.mapURL.value == ''? dataHotel.mapURL : target.mapURL.value,
        ownerID : ownerID,
      ...
      console.log(jason);
      const res =
        await apiClient(`${config.api_url.localHost}/Hotel/${id}`,{method : 'PUT',headers :{"Content-Type" : "application/json"} ,data : jason})
        console.log("ok");
